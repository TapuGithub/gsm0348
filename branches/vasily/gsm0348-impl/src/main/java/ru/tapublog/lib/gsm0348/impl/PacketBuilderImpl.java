...
public class PacketBuilderImpl implements PacketBuilder
{
	...
	@Override
	public CommandPacket recoverCommandPacket(byte[] rawdata, byte[] cipheringKey, byte[] signatureKey)
			throws PacketBuilderConfigurationException, Gsm0348Exception
	{
        if (LOGGER.isDebugEnabled()) {
        ...
        try {
            if (commandPacketCiphering) {
                ...
                byte[] dataEnc = CipheringManager.decipher(cipheringAlgorithmName, cipheringKey, dataCiph);
                System.arraycopy(dataEnc, 0, counters, 0, COUNTERS_SIZE);
                paddingCounter = Util.unsignedByteToInt(dataEnc[COUNTERS_SIZE]);
                ...
                final int deltaDataSize = COUNTERS_SIZE + PADDING_COUNTER_SIZE + signatureLength;
                data = new byte[dataCiph.length - deltaDataSize - paddingCounter];
                System.arraycopy(dataEnc, deltaDataSize, data, 0, dataEnc.length - deltaDataSize);
            } else {
            ...
            if (commandPacketSigning) {
                ...
                signData[addonAmount + SPI_POSITION] = CommandSPICoder.decode(cardProfile.getSPI().getCommandSPI());
                signData[addonAmount + SPI_POSITION + 1] = ResponseSPICoder.decode(cardProfile.getSPI().getResponseSPI());
                signData[addonAmount + KIC_POSITION] = KICCoder.decode(cardProfile.getKIC());
                signData[addonAmount + KID_POSITION] = KIDCoder.decode(cardProfile.getKID());
			...
			CommandPacketHeader pacHeader = new CommandPacketHeader();
            pacHeader.setKIC(cardProfile.getKIC());
            pacHeader.setKID(cardProfile.getKID());
            pacHeader.setSPI(cardProfile.getSPI());
			pacHeader.setTAR(cardProfile.getTAR());
			pacHeader.setCounter(counters);
			pacHeader.setPaddingCounter((byte) paddingCounter);
			pacHeader.setSecurity(signature);
            ...
            commandPacket.setHeader(pacHeader);
