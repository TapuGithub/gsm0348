...
public class PacketBuilderImpl implements PacketBuilder {
    ...
    private CardProfile copyProfile(CardProfile profile) throws Gsm0348Exception {
        try {
            KIC a = new KIC();
            return (CardProfile) profile.clone();
        } catch (Exception ex) {
        ...
        switch (kic.getAlgorithmImplementation()) {
            ...
            case DES:
                switch (kic.getCipheringAlgorithmMode()) {
                    case DES_CBC:
                        cipheringAlgorithmName = "DES/CBC/NoPadding"; //DES/CBC/ZeroBytePadding
                        break;
    ...
    public byte[] buildCommandPacket(byte[] data, byte[] counters, byte[] cipheringKey, byte[] signatureKey)
        ...
        try {
            ...
            if (commandPacketCiphering) {
                ...
                byte[] cipheredData = CipheringManager.encipher(cipheringAlgorithmName,
                        cipheringKey, cipherData, new byte[]{0, 0, 0, 0, 0}); //countersBytes
                if (LOGGER.isDebugEnabled()) {
    ...
    @Override
    public byte[] recoverCommandPacket(byte[] rawdata, byte[] cipheringKey, byte[] signatureKey)
            throws PacketBuilderConfigurationException, Gsm0348Exception {
        if (LOGGER.isDebugEnabled()) {
        ...
        final int packetLength = Util.unsignedByteToInt(rawdata[0]) * 0xff + Util.unsignedByteToInt(rawdata[1]);
        ...
        final int headerLength = Util.unsignedByteToInt(rawdata[HEADER_LENGHT_POSITION + 2]);
        ...
        spi.setValue(Arrays.copyOfRange(rawdata, 2 + SPI_POSITION, 2 + SPI_POSITION + SPI_SIZE));
        kic.setValue(rawdata[2 + KIC_POSITION]);
        kid.setValue(rawdata[2 + KID_POSITION]);
        
        cardProfile.setKIC(kic);
        cardProfile.setKID(kid);
        cardProfile.setSPI(spi);
        cardProfile.setTAR(Arrays.copyOfRange(rawdata, 2 + TAR_POSITION, 2 + TAR_POSITION + TAR_SIZE));
        
        if (commandPacketCiphering && (cipheringKey == null || cipheringKey.length == 0)) {
            if(cardProfile.getKIC().getKeysetID() < 1)
                throw new PacketBuilderConfigurationException("Command ciphering is enabled - ciphering key must be specified. Provided: " + ((cipheringKey.length == 0) ? "empty" : Util.toHexArray(cipheringKey)));
            cipheringKey = cardProfile.getCipheringKey(cardProfile.getKIC().getKeysetID() - 1);
            if(cipheringKey == null || cipheringKey.length == 0)
                throw new PacketBuilderConfigurationException("Command ciphering is enabled - ciphering key must be specified. Provided: " + ((cipheringKey.length == 0) ? "empty" : Util.toHexArray(cipheringKey)));
        }
        if (commandPacketSigning && (signatureKey == null || signatureKey.length == 0)) {
            if(cardProfile.getKID().getKeysetID() < 1)
                throw new PacketBuilderConfigurationException("Command signing is enabled - signature key must be specified. Provided: " + ((signatureKey.length == 0) ? "empty" : Util.toHexArray(signatureKey)));
            signatureKey = cardProfile.getSignatureKey(cardProfile.getKID().getKeysetID() - 1);
        ...
        if (rawdata.length < MINIMUM_COMMAND_PACKET_SIZE + signatureLength) {
            String message = "rawdata too small to be command packet. Expected to be >= "
                    + (MINIMUM_COMMAND_PACKET_SIZE + signatureLength) + ", but found " + rawdata.length;
            if (rawdata.length >= MINIMUM_COMMAND_PACKET_SIZE) {
                message += ". It can be caused by incorrect profile(SPI value). Check SPI!";
                if (LOGGER.isEnabledFor(Level.WARN)) {
        ...
        final byte[] signature = new byte[signatureLength];
        int paddingCounter = Util.unsignedByteToInt(rawdata[PADDING_COUNTER_POSITION + 2]);
        if (!commandPacketCiphering && paddingCounter != 0) {
            throw new Gsm0348Exception(
                    "Command packet ciphering is off but padding counter isnot 0. So it can be corrupted packet or configuration doesn`t match provided data");
        }
        byte[] data;
        byte[] dataEnc = null;
        try {
            if (commandPacketCiphering) {
                byte[] dataCiph = Arrays.copyOfRange(rawdata, 2 + HEADER_SIZE_UNCRIPTED, rawdata.length);
                dataEnc = CipheringManager.decipher(cipheringAlgorithmName, cipheringKey, dataCiph);
                System.arraycopy(dataEnc, 0, counters, 0, COUNTERS_SIZE);
                paddingCounter = Util.unsignedByteToInt(dataEnc[COUNTERS_SIZE]);
                if (dataEnc.length < COUNTERS_SIZE + 1 + signatureLength) {
                    throw new Gsm0348Exception("Packet recovery failure. Possibly because of unexpected security bytes length. Expected: " + signatureSize);
                }
                System.arraycopy(dataEnc, COUNTERS_SIZE + 1, signature, 0, signatureLength);
                final int dataSize = dataEnc.length - COUNTERS_SIZE - PADDING_COUNTER_SIZE - signatureLength - paddingCounter;
                data = new byte[dataSize];
                System.arraycopy(dataEnc, COUNTERS_SIZE + PADDING_COUNTER_SIZE + signatureLength, data, 0, dataSize);
            } else {
                System.arraycopy(rawdata, 2 + COUNTERS_POSITION, counters, 0, COUNTERS_SIZE);
                paddingCounter = Util.unsignedByteToInt(rawdata[2 + PADDING_COUNTER_POSITION]);
                System.arraycopy(rawdata, 2 + SIGNATURE_POSITION, signature, 0, signatureLength);
                final int dataSize = packetLength - headerLength;
                data = new byte[dataSize];
                System.arraycopy(rawdata, headerLength + PACKET_LENGHT_SIZE, data, 0, dataSize);
            }
            ...
            if (commandPacketSigning) {
                ...
                int signDataSize = 1 + addonAmount + SPI_SIZE
                        + KID_SIZE + KIC_SIZE
                        + TAR_SIZE + PADDING_COUNTER_SIZE
                        + COUNTERS_SIZE + data.length;
                int signDataAddSize = signDataSize%8;
                if(signDataAddSize > 0) {
                    signDataAddSize = 8 - signDataAddSize;
                    signDataSize += signDataAddSize;
                }
                //paddingCounter = (PADDING_COUNTER_SIZE + COUNTERS_SIZE + data.length)%8;
                //if(paddingCounter > 0)
                //    paddingCounter = 8 - paddingCounter;
                        
                byte[] signData = new byte[signDataSize];
                switch (cardProfile.getSecurityBytesType()) {
                    case WITH_LENGHTS_AND_UDHL:
                        signData[0] = 0x02;
                        signData[1] = 0x71;
                ...
                System.arraycopy(cardProfile.getSPI().getValue(), 0, signData, addonAmount + SPI_POSITION, SPI_SIZE);
                signData[addonAmount + KIC_POSITION] = cardProfile.getKIC().getValue();
                signData[addonAmount + KID_POSITION] = cardProfile.getKID().getValue();
                System.arraycopy(cardProfile.getTAR(), 0, signData, addonAmount + TAR_POSITION, TAR_SIZE);
                System.arraycopy(counters, 0, signData, addonAmount + COUNTERS_POSITION, COUNTERS_SIZE);
                signData[addonAmount + PADDING_COUNTER_POSITION] = (byte) paddingCounter;
                System.arraycopy(data, 0, signData, addonAmount + PADDING_COUNTER_POSITION + 1, data.length);
            ...
            headerData[HEADER_LENGHT_POSITION] = (byte) (headerLength);
            System.arraycopy(spi, 0, headerData, SPI_POSITION, SPI_SIZE);
            headerData[KIC_POSITION] = kic;
            headerData[KID_POSITION] = kid;
            System.arraycopy(tar, 0, headerData, TAR_POSITION, TAR_SIZE);
            System.arraycopy(counters, 0, headerData, COUNTERS_POSITION, COUNTERS_SIZE);
            headerData[PADDING_COUNTER_POSITION] = (byte) paddingCounter;
            System.arraycopy(signature, 0, headerData, SIGNATURE_POSITION, signatureLength);
            
            CommandPacketHeader header = new CommandPacketHeaderImpl(headerData, signatureLength);
            CommandPacket packet = new CommandPacketImpl(header, new PacketDataImpl(data));
            if (LOGGER.isDebugEnabled())
